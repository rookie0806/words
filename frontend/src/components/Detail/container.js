...
class Container extends Component {
  ...
  componentWillReceiveProps = nextProps => {
    ...
    if(nextProps.failtest){
      ...
      if(this.state.failtestopen==true){
        ...
        if(this.state.failtestwithkor==true){
          var win = window.open("/test/"+nextProps.failtest.status+"?kor=yes");
        }
        else{
          var win = window.open("/test/"+nextProps.failtest.status+"?kor=no");
        }
  ...
  makeFailTestWithAnswerClick = (e) =>{
    const{makeFailTest} = this.props;
    makeFailTest(this.state.stduuid);
    this.setState({
      failtestopen:true,
      failtestwithkor : true,
    })
  }
  handle = (e) =>{
